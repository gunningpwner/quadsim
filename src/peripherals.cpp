...
void SystemClock_Config_HSE(void)
{
  ...
  // This puts the system clock @ 168MHz
...
void MX_DMA_Init(void)
{
  ...
  /* DMA1_Stream2_IRQn interrupt configuration (for UART4_RX) */
  HAL_NVIC_SetPriority(DMA1_Stream2_IRQn, 1, 0); // Lower priority than IMU
...
void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim)
{
  ...
  else if (htim->Instance == TIM8)
  {
    ...
    PC8     ------> TIM8_CH3 --> Motor 1
    PC9     ------> TIM8_CH4 --> Motor 3
    */
...
void MX_TIM2_Init(void)
{
  ...
  htim2.Instance = TIM2;
  htim2.Init.Prescaler = 420 - 1; // Gives a 100kHz timer clock
  htim2.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim2.Init.Period = 125 - 1; // Count up to 1000 for a 10ms period (100Hz)
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *spiHandle)
{
  ...
  else if (spiHandle->Instance == SPI2)
  {
    ...
    /* 2. Configure GPIOs (SCK, MISO, MOSI) */
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
  ...
  else if (huart->Instance == UART4)
  {
    ...
    */
    ...
    HAL_NVIC_SetPriority(UART4_IRQn, 1, 0); // Lower priority than IMU
...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *tim_baseHandle)
{
  ...
  else if (tim_baseHandle->Instance == TIM8)
  {
    ...
    HAL_DMA_Init(&hdma_tim8_ch3);

    __HAL_LINKDMA(tim_baseHandle, hdma[TIM_DMA_ID_CC3], hdma_tim8_ch3);

    hdma_tim8_ch4.Instance = DMA2_Stream7;
    hdma_tim8_ch4.Init.Channel = DMA_CHANNEL_7;
