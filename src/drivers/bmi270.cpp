...
BMI270::BMI270(DataManager::SensorBuffer &m_buffer, SPI_HandleTypeDef *spi_handle, GPIO_TypeDef *cs_port, uint16_t cs_pin)
    : m_sensor_buffer(m_buffer), m_spi_handle(spi_handle), m_cs_port(cs_port), m_cs_pin(cs_pin) {}
...
int8_t BMI270::init(void)
{
  ...
  for (int i = 0; i < sizeof(bmi270_config_file); i++)
  {
    ...
    {
      if (mismatches == 0)
        printf("Mismatched bytes found:\n");
      printf("Index %d: Wrote 0x%02X, Read 0x%02X\n", i, bmi270_config_file[i], test[i]);
      mismatches++;
    }
  }
  if (mismatches == 0)
    printf("Config file verified successfully!\n");
  delay_us(10);
#endif
